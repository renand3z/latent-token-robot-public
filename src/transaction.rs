...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let keypair = read_keypair_file("swap_wallet.json").unwrap_or_else(|err| {
        println!("Failed to read `swap_wallet.json`: {err}");
        println!("An ephemeral keypair will be used instead. For a more realistic example, create a new keypair at that location and fund it with a small amount of SOL.");
        Keypair::new()
    });
